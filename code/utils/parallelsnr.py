...
class ParallelSNR:
    ...
    def __init__(self, m1, m2, z, st, et, chi=0.8, chi_1=None, chi_2=None,
                 snr_kwargs={
                            'wd_noise': True, 'num_points': 2048, 'sensitivity_curve': 'LPA',
                            'prefactor': np.sqrt(16./5.)}):
    ...
    def prep_parallel(self, num_processors=None, num_splits=1000, verbose=50):
        ...
        for i, ind_split in enumerate(inds_split_all):
            snr_args = (
                        self.m1[ind_split], self.m2[ind_split],
                        self.chi_1[ind_split], self.chi_2[ind_split], self.z[ind_split],
                        self.st[ind_split], self.et[ind_split])
