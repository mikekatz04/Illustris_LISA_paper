...
class KDEResample:
    ...
    def resample(self, size=1):
        ...
        norm = rm.multivariate_normal(
            np.zeros((self.data.T.shape[0],), float), self.covariance, size=size)

        indices = rm.choice(
            np.arange(len(self.data)), size=size, replace=True, p=self.weights/self.weights.sum())
