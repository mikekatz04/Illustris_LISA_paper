...
class Disk_Torque(Hardening_Mechanism):
    ...
    def harden(self, rads, m1, m2, mdot):
...
def time_inspiral(rads, m1, m2, mdot, sets, regs=None, qs=None, vs=None):
    lam = sets.DISK_GAP_RAD
    if regs is None:
        regs = _rad_regions(rads, m1+m2, mdot, sets)

    # Disk dominance parameter (modulates viscous-time)
    if qs is None:
        qs = qdom(rads, m1, m2, mdot, sets, regs=regs)
    q1, q2, q3, qb = qs
    # Viscous-Timescale evaluated at binary position (relevant for disk-dominated regions)
    if vs is None:
        vs = tvisc(rads, m1+m2, mdot, sets, regs=regs)
    v1, v2, v3, vt = vs
    ...
    if np.isclose(lam, 1.0):
        ...
        v1_l, v2_l, v3_l = tvisc(rads*lam, m1+m2, mdot, sets, regs=regs)
    ...
    t1, t2, t3 = _inspiral_time([v1, v2, v3], [v1_l, v2_l, v3_l], [q1, q2, q3])
    ti = _merge_regions(regs, np.inf*np.ones_like(rads), t1, t2, t3, None)

    return t1, t2, t3, ti


def tvisc(rads, mtot, mdot, sets, regs=None):
    ...
    """
    lam = sets.DISK_GAP_RAD
    tmu_0 = sets.MEAN_MOL_WEIGHT_TIL_MU_0
    tmu_e = sets.MEAN_MASS_PER_ELEC_TIL_MU_E
    tf_t = sets.TEMP_OPAC_CONST_TIL_F_T
    tkap_es = sets.KAPPA_ES
    tkap_ff = sets.KAPPA_FF
    alpha_03 = sets.ALPHA_VISC_03
    eps_01 = sets.RAD_EFF_EPS_01

    rads = np.atleast_1d(rads)
    if regs is None:
        regs = _rad_regions(rads, mtot, mdot, sets)

    v1 = _tvisc_1(rads, lam, mtot, mdot,
                  tmu_e=tmu_e, tf_t=tf_t, tkap_es=tkap_es,
                  alpha_03=alpha_03, eps_01=eps_01)
    v2 = _tvisc_2(rads, lam, mtot, mdot,
                  tmu_0=tmu_0, tmu_e=tmu_e, tf_t=tf_t, tkap_es=tkap_es,
                  alpha_03=alpha_03, eps_01=eps_01)
    v3 = _tvisc_3(rads, lam, mtot, mdot,
                  tmu_0=tmu_0, tmu_e=tmu_e, tf_t=tf_t, tkap_ff=tkap_ff,
                  alpha_03=alpha_03, eps_01=eps_01)

    vs = _merge_regions(regs, np.inf*np.ones_like(rads), v1, v2, v3, None)
    return v1, v2, v3, vs


def qdom(rads, m1, m2, mdot, sets, regs=None):
    ...
    q1 = _qdom_1(rads, lam, m1, m2, mdot,
                 tmu_e=tmu_e, tf_t=tf_t, tkap_es=tkap_es,
                 alpha_03=alpha_03, eps_01=eps_01)
    q2 = _qdom_2(rads, lam, m1, m2, mdot,
                 tmu_0=tmu_0, tmu_e=tmu_e, tf_t=tf_t, tkap_es=tkap_es,
                 alpha_03=alpha_03, eps_01=eps_01)
    q3 = _qdom_3(rads, lam, m1, m2, mdot,
                 tmu_0=tmu_0, tmu_e=tmu_e, tf_t=tf_t, tkap_ff=tkap_ff,
                 alpha_03=alpha_03, eps_01=eps_01)

    qb = _merge_regions(regs, np.ones_like(rads), q1, q2, q3, None)
    return q1, q2, q3, qb


def _self_grav(regs, tinsp, sets):
    """Set inspiral timescale in Self-Gravity unstable region to infinite.
    """
    sg_rad, sg_rad_max = _parse_sg_rads(sets)
    if sg_rad > 0.0:
        tinsp[regs.inds[3]] = np.inf
    return tinsp


def _inspiral_time(tvs, tvs_lam, qbs):
...
def _mdot_edd(tmass, eps_01=1.0, tmu_e=1.0):
...
def _rad_regions(rads, tmass, mdot, sets):
    ...
    cr_12 = _rad_crit_12(tmass, mdot,
                         tmu_0=tmu_0, tmu_e=tmu_e, tkap_es=tkap_es,
                         alpha_03=alpha_03, eps_01=eps_01)
    cr_23 = _rad_crit_23(tmass, mdot,
    ...
    rmap = np.zeros_like(rads, dtype=int)
    ...
    if sg_rad > 0.0:
        ...
        cr_sg = sg_rad * _rad_crit_sg(cr_sg_es, cr_sg_ff, _inds2, _inds3)
        ...
        if not sets.SG_MAX_REG1 and num_reg1 != np.count_nonzero(rmap == 1):
...
def _rad_crit_12(tmass, mdot, tmu_0=1.0, tmu_e=1.0, tkap_es=1.0, alpha_03=1.0, eps_01=1.0):
    ...
    """
    _r3 = 1e3*physics.schwarzschildRadius(tmass)
    _mdot = mdot/(0.1*_mdot_edd(tmass, eps_01, tmu_e))
...
def _rad_crit_23(tmass, mdot, tmu_0=1.0, tmu_e=1.0, tf_t=1.0,
...
def _rad_crit_sg_es(tmass, mdot, tmu_0=1.0, tmu_e=1.0, tf_t=1.0,
...
def _rad_crit_sg_ff(tmass, mdot, tmu_0=1.0, tmu_e=1.0, tf_t=1.0,
                    tkap_es=1.0, tkap_ff=1.0, alpha_03=1.0, eps_01=1.0):
...
def _rad_crit_sg(cr_sg_es, cr_sg_ff, inds2, inds3):
    ...
    cr_sg = np.inf * np.ones_like(cr_sg_es)
    # Set values in regions 2 and 3 appropriately
    cr_sg[inds2] = cr_sg_es[inds2]
    cr_sg[inds3] = cr_sg_ff[inds3]
    return cr_sg


def _qdom_1(rad, lam, m1, m2, mdot, tmu_e=1.0, tf_t=1.0, tkap_es=1.0, alpha_03=1.0, eps_01=1.0):
    """Disk Dominance Parameter q_B in region 1.
    """
    tmass = m1+m2  # Total mass
    _mde = mdot/(0.1*_mdot_edd(tmass, eps_01, tmu_e))  # MDot in units of 0.1 Eddington
    _tmass = tmass/(1.0e7*MSOL)  # Total Mass in units of $1e7*Msol$
    _r3 = rad/(1e3*physics.schwarzschildRadius(tmass))  # Radius in units of $1e3*R_s$
    qs = _mass_ratio_sym(m1, m2)  # Normal and symmetric mass-ratios
    qb1 = 1.2e-3*np.power(tmu_e*alpha_03, -1.)*np.power(tkap_es*tf_t, -2.)
    qb1 *= (_tmass/_mde)*np.power(lam*_r3, 7./2)/qs
    return qb1


def _qdom_2(rad, lam, m1, m2, mdot, tmu_0=1.0, tmu_e=1.0, tf_t=1.0,
...
def _qdom_3(rad, lam, m1, m2, mdot, tmu_0=1.0, tmu_e=1.0, tf_t=1.0,
...
def _tvisc_1(rad, lam, tmass, mdot, tmu_e=1.0, tf_t=1.0, tkap_es=1.0, alpha_03=1.0, eps_01=1.0):
...
def _tvisc_2(rad, lam, tmass, mdot, tmu_0=1.0, tmu_e=1.0, tf_t=1.0,
             tkap_es=1.0, alpha_03=1.0, eps_01=1.0):
...
def _tvisc_3(rad, lam, tmass, mdot, tmu_0=1.0, tmu_e=1.0, tf_t=1.0,
             tkap_ff=1.0, alpha_03=1.0, eps_01=1.0):
...
def _parse_sg_rads(sets):
    ...
    """
    sg_rad = sets.SELF_GRAV_RAD
    sg_rad_max = sets.SELF_GRAV_RAD_MAX
...
def _merge_regions(regs, vals, v1, v2, v3, v4):
